...
function ZyButton:new(picNor, picDown, picDis, title, fontName, fontSize)
	...
	elseif picNor then
		...
		if title then
			...
			label1 = CCLabelTTF:create(title, size, kCCTextAlignmentCenter, fontName, fontSize)
			label1:setPosition(CCPoint(spriteNor:getContentSize().width / 2, spriteNor:getContentSize().height / 2))
			spriteNor:addChild(label1, 0)
			
			label2 = CCLabelTTF:create(title, size, kCCTextAlignmentCenter, fontName, fontSize)
...
function ZyButton:addImage(image)
	if self._menuFont then
		self._menuFont:getParent():removeChild(self._menuFont ,true)
		self._menuFont=nil
	end
	local sprite=CCSprite:create(P(image))
	sprite:setAnchorPoint(CCPoint(0.5,0.5))
	sprite:setPosition(CCPoint(self._menu:getContentSize().width/2,
								self._menu:getContentSize().height/2))
	self._menuFont=sprite
	self._menuItem:addChild(sprite,1)
end