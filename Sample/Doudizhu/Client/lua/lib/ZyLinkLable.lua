...
function ZyLinkLable:new(text,color,fontname,fontsize,width,isLine)
	...
	--for i=1,nRow do
	--    local lineNode = NdLineNode:lineWithPoint(PT(0,nHeight*(nRow-i)), PT(lengthList[i], nHeight*(nRow-i)), 1, color)
...
function ZyLinkLable:setIsVisible(visible)
	self._menu:setIsVisible(visible)
end
