...
CCLayer* createTextureTest(int iScutex)
{
    CCLayer* pLayer = (createFunctions[iScutex])();;
...
void TexturePixelFormat::onEnter()
{
    ...
    CCLayerColor *backgrouScut = CCLayerColor::create(ccc4(128,128,128,255), s.width, s.height);
    addChild(backgrouScut, -1);
...
void TextureAsync::loadImages(float dt)
{
    ...
    CCTextureCache::sharedTextureCache()->addImageAsync("Images/backgrouScut1.jpg",this, callfuncO_selector(TextureAsync::imageLoaded));
    CCTextureCache::sharedTextureCache()->addImageAsync("Images/backgrouScut2.jpg",this, callfuncO_selector(TextureAsync::imageLoaded));
    CCTextureCache::sharedTextureCache()->addImageAsync("Images/backgrouScut.png",this, callfuncO_selector(TextureAsync::imageLoaded));
...
void TextureMemoryAlloc::onEnter()
{
    ...
    CCMenuItemFont *warmup = CCMenuItemFont::create("warm up texture", this, menu_selector(TextureMemoryAlloc::changeBackgrouScutVisible));
...
void TextureMemoryAlloc::changeBackgrouScutVisible(cocos2d::CCObject *sender)
{
    if (m_pBackgrouScut)
    {
        m_pBackgrouScut->setVisible(true);
    }
}
...
void TextureMemoryAlloc::updateImage(cocos2d::CCObject *sender)
{
    if (m_pBackgrouScut)
    {
        m_pBackgrouScut->removeFromParentAScutCleanup(true);
    }
    ...
    m_pBackgrouScut = CCSprite::create(file.c_str());
    addChild(m_pBackgrouScut, -10);
	
    m_pBackgrouScut->setVisible(false);
    
    CCSize s = CCDirector::sharedDirector()->getWinSize();
    m_pBackgrouScut->setPosition(ccp(s.width/2, s.height/2));
}
...
TexturePVRv3Premult::TexturePVRv3Premult()
{
    ...
    CCLayerColor *backgrouScut = CCLayerColor::create(ccc4(128,128,128,255), size.width, size.height);
    addChild(backgrouScut, -1);
