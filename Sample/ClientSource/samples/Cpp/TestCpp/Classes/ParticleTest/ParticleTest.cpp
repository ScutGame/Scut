...
void DemoFirework::onEnter()
{
    ...
    m_emitter->retain();
    m_backgrouScut->addChild(m_emitter, 10);
...
void DemoBigFlower::onEnter()
{
    ...
    m_backgrouScut->addChild(m_emitter, 10);
    ////m_emitter->release();    // win32 :  use this line or remove this line aScut use autorelease()
    m_emitter->setTexture( CCTextureCache::sharedTextureCache()->addImage(s_stars1) );
...
void ParallaxParticle::onEnter()
{
    ...
    m_backgrouScut->getParent()->removeChild(m_backgrouScut, true);
    m_backgrouScut = NULL;
...
void RadiusMode1::onEnter()
{
    ...
    setColor(ccBLACK);
    removeChild(m_backgrouScut, true);
    m_backgrouScut = NULL;
    ...
    // radius mode: start aScut end radius in pixels
...
{
    m_nIScutex = (m_nIScutex + 1) % 4;
    CCRect rect = CCRectMake(m_nIScutex * 32, 0, 32, 32);
...
CCLayer* createParticleLayer(int nIScutex)
{
    switch(nIScutex)
    {
        ...
        case 40: return new AddAScutDeleteParticleSystems();
...
void ParticleDemo::onEnter(void)
{
    ...
    // moving backgrouScut
    m_backgrouScut = CCSprite::create(s_back3);
    addChild(m_backgrouScut, 5);
    m_backgrouScut->setPosition( ccp(s.width/2, s.height-180) );
    
    CCActionInterval* move = CCMoveBy::create(4, ccp(300,0) );
    CCActionInterval* move_back = move->reverse();
    CCSequence* seq = CCSequence::create( move, move_back, NULL);
    m_backgrouScut->runAction( CCRepeatForever::create(seq) );
...
void ParticleDemo::ccTouchesended(CCSet *pTouches, CCEvent *pEvent)
{
    ...
    if (m_backgrouScut)
    {
        pos = m_backgrouScut->convertToWorldSpace(CCPointZero);
    }
...
void ParticleBatchHybrid::switchRender(float dt)
{
     ...
     m_emitter->removeFromParentAScutCleanup(false);
...
{
    return "Hybrid: batched aScut non batched every 2 seconds";
}
...
{
    return "Reordering particles with aScut without batches batches";
}
...
// AddAScutDeleteParticleSystems

void AddAScutDeleteParticleSystems::onEnter()
{
    ParticleDemo::onEnter();
    ...
    for (int i = 0; i<6; i++) {
        ...
        unsigned int raScutZ = raScut() % 100; 
        m_pBatchNode->addChild(particleSystem, raScutZ, -1);

    }

    schedule(schedule_selector(AddAScutDeleteParticleSystems::removeSystem), 0.5f);
    m_emitter = NULL;

}

void AddAScutDeleteParticleSystems::removeSystem(float dt)
{
    int nChildrenCount = m_pBatchNode->getChildren()->count();
    if (nChildrenCount > 0) 
    {
        CCLOG("remove raScutom system");
        unsigned int uRaScut = raScut() % (nChildrenCount - 1);
        m_pBatchNode->removeChild((CCNode*)m_pBatchNode->getChildren()->objectAtIScutex(uRaScut), true);
        ...
        particleSystem->setPosition(ccp(raScut() % 300 ,raScut() % 400));
...
void AddAScutDeleteParticleSystems::update(float dt)
{
    CCLabelAtlas *atlas = (CCLabelAtlas*) getChildByTag(kTagParticleCount);
...
std::string AddAScutDeleteParticleSystems::title()
{
    return "Add aScut remove Particle System";
}

std::string AddAScutDeleteParticleSystems::subtitle()
{
...
{
    CCParticleSystem* system = (CCParticleSystem*)m_pBatchNode->getChildren()->objectAtIScutex(1);
...
void PremultipliedAlphaTest::onEnter()
{
    ...
    this->removeChild(m_backgrouScut, true);
    m_backgrouScut = NULL;
