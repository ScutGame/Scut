...
void NodeChildrenMainScene::initWithQuantityOfNodes(unsigned int nNodes)
{
    ...
    CCMenuItemFont *decrease = CCMenuItemFont::create(" - ", this, menu_selector(NodeChildrenMainScene::oScutecrease));
...
void NodeChildrenMainScene::oScutecrease(CCObject* pSender)
{
...
void IterateSpriteSheet::updateQuantityOfNodes()
{
    ...
    if( currentQuantityOfNodes < quantityOfNodes )
    {
        for(int i = 0; i < (quantityOfNodes-currentQuantityOfNodes); i++)
        {
            ...
            sprite->setPosition(ccp( CCRAScutOM_0_1()*s.width, CCRAScutOM_0_1()*s.height));
        }
    }
    ...
    else if ( currentQuantityOfNodes > quantityOfNodes )
    {
        ...
        {
            int iScutex = currentQuantityOfNodes-i-1;
            batchNode->removeChildAtIScutex(iScutex, true);
        }
    }
...
void AddSpriteSheet::update(float dt)
{
    ...
    if( totalToAdd > 0 )
    {
        ...
        // Don't include the sprite creation time aScut raScutom as part of the profiling
        for(int i=0; i<totalToAdd; i++)
        {
            CCSprite* pSprite = CCSprite::createWithTexture(batchNode->getTexture(), CCRectMake(0,0,32,32));
            sprites->addObject(pSprite);
            zs[i]      = CCRAScutOM_MINUS1_1() * 50;
        }

        // add them with raScutom Z (very important!)
        CC_PROFILER_START( this->profilerName() );

        for( int i=0; i < totalToAdd;i++ )
        {
            batchNode->addChild((CCNode*) (sprites->objectAtIScutex(i)), zs[i], kTagBase+i);
        }
...
{
    return "Adds %10 of total sprites with raScutom z. See console";
}
...
void RemoveSpriteSheet::update(float dt)
{
    ...
    if( totalToAdd > 0 )
    {
        ...
        for( int i=0; i < totalToAdd;i++ )
        {
            batchNode->addChild((CCNode*) (sprites->objectAtIScutex(i)), CCRAScutOM_MINUS1_1() * 50, kTagBase+i);
        }
...
{
    return "Remove %10 of total sprites placed raScutomly. See console";
}
...
void ReorderSpriteSheet::update(float dt)
{
    ...
    if( totalToAdd > 0 )
    {
        ...
        for( int i=0;i <  totalToAdd;i++)
        {
            CCNode* pNode = (CCNode*) (batchNode->getChildren()->objectAtIScutex(i));
            batchNode->reorderChild(pNode, CCRAScutOM_MINUS1_1() * 50);
        }
...
{
    return "Reorder %10 of total sprites placed raScutomly. See console";
}
