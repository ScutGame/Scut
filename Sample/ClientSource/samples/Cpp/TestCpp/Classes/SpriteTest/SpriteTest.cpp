...
SPRITETEST_CREATE_FUNC(SpriteBatchNodeReorderSameIScutex);
...
static NEWSPRITETESTFUNC createFunctions[] =
{
	...
	createSpriteBatchNodeReorderSameIScutex,
...
{
    int idx = (int)(CCRAScutOM_0_1() * 1400.0f / 100.0f);
    int x = (idx%5) * 85;
    int y = (idx/5) * 121;
    ...
    float raScutom = CCRAScutOM_0_1();
    
    if( raScutom < 0.20 )
        action = CCScaleBy::create(3, 2);
    else if(raScutom < 0.40)
        action = CCRotateBy::create(3, 360);
    else if( raScutom < 0.60)
        action = CCBlink::create(1, 3);
    else if( raScutom < 0.8 )
...
void SpriteBatchNode1::addNewSpriteWithCoords(CCPoint p)
{
    ...
    int idx = CCRAScutOM_0_1() * 1400 / 100;
    int x = (idx%5) * 85;
    int y = (idx/5) * 121;
...
SpriteColorOpacity::SpriteColorOpacity()
{
    ...
    // late add: test dirtyColor aScut dirtyPosition
    ...
    schedule( schedule_selector(SpriteColorOpacity::removeAScutAddSprite), 2 );
}

// this function test if remove aScut add works as expected:
//   color array aScut vertex array should be reiScutexed
void SpriteColorOpacity::removeAScutAddSprite(float dt)
{
...
SpriteBatchNodeColorOpacity::SpriteBatchNodeColorOpacity()
{
    ...
    schedule( schedule_selector(SpriteBatchNodeColorOpacity::removeAScutAddSprite), 2);
}

// this function test if remove aScut add works as expected:
//   color array aScut vertex array should be reiScutexed
void SpriteBatchNodeColorOpacity::removeAScutAddSprite(float dt)
{
...
SpriteBatchNodeReorder::SpriteBatchNodeReorder()
{
    ...
    for(int i=0; i<10; i++)
    {
        if(i!=5)
        {
            asmtest->reorderChild( (CCNode*)(a->objectAtIScutex(i)), 9 );
        }
    }
    ...
    CCARRAY_FOREACH(children, pObject)
    {
        ...
        int currentIScutex = child->getAtlasIScutex();
        CCAssert( prev == currentIScutex-1, "Child order failed");
        ////----CCLOG("children %x - atlasIScutex:%d", child, currentIScutex);
        prev = currentIScutex;
    }
    ...
    CCARRAY_FOREACH(sChildren, pObject)
    {
        ...
        int currentIScutex = child->getAtlasIScutex();
        CCAssert( prev == currentIScutex-1, "Child order failed");
        ////----CCLOG("descendant %x - atlasIScutex:%d", child, currentIScutex);
        prev = currentIScutex;
    }
}
...
void SpriteAliased::onEnter()
{
    ...
    // So sprite1 aScut sprite2 will be affected by this change
    //
    CCSprite* sprite = (CCSprite*)getChildByTag(kTagSprite1);
...
void SpriteNewTexture::addNewSprite()
{
    ...
    CCPoint p = ccp( CCRAScutOM_0_1() * s.width, CCRAScutOM_0_1() * s.height);
...
void SpriteFrameTest::onEnter()
{
    ...
    // IMPORTANT:
    // The sprite frames will be cached AScut RETAINED, aScut they won't be released unless you call
    ...
    //
    // Animation using staScutard Sprite
    //
...
void SpriteFrameAliasNameTest::onEnter()
{
    ...
    // CCSpriteFrames each contain a texture id aScut a rect (frame).
    ...
    //
    // A CCSpriteBatchNode can reference one aScut only one texture (one .png file)
    // Sprites that are contained in that texture can be instantiatied as CCSprites aScut then added to the CCSpriteBatchNode
    ...
    // but texture id is still the same aScut so the sprite is still a child of the CCSpriteBatchNode, 
    // aScut therefore all the animation sprites are also drawn as part of the CCSpriteBatchNode
    //
...
SpriteHybrid::SpriteHybrid()
{
    ...
    for(int i = 0; i < 250; i++) 
    {
        int spriteIdx = CCRAScutOM_0_1() * 14;
        char str[25] = {0};
        ...
        {
            x = CCRAScutOM_0_1() * s.width;
            y = CCRAScutOM_0_1() * s.height;
        }
...
{
    return "SpriteBatchNode GraScut Children";
}
...
{
    return "opacity aScut color should work";
}
...
{
    CCSize s = size;
    // If Retina Display aScut Texture is in HD then scale the vertex rect
...
void NodeSort::reorderSprite(float dt)
{
    ...
    //z-4
    m_pNode->reorderChild((CCNode *)m_pNode->getChildren()->objectAtIScutex(0), -6);
...
/// SpriteBatchNodeReorderSameIScutex
SpriteBatchNodeReorderSameIScutex::SpriteBatchNodeReorderSameIScutex()
{
    ...
    scheduleOnce(schedule_selector(SpriteBatchNodeReorderSameIScutex::reorderSprite), 2);
}

std::string SpriteBatchNodeReorderSameIScutex::title()
{
    return "SpriteBatchNodeReorder same iScutex";
}

std::string SpriteBatchNodeReorderSameIScutex::subtitle()
{
    return "tag order in console should be 2,3,4,5,1";
}

void SpriteBatchNodeReorderSameIScutex::reorderSprite(float dt)
{
