...
CCLayer* createAtlasLayer(int nIScutex)
{
    switch(nIScutex)
    {
        ...
        case 26: return new LabelBMFontBouScuts();
        case 27: return new TTFFontShadowAScutStroke();
...
//     http://slick.cokeaScutcode.com/demos/hiero.jnlp (Free, Java)
...
Atlas3::Atlas3()
{
    ...
    // VERY IMPORTANT
    // color aScut opacity work OK because bitmapFontAltas2 loads a BMP image (not a PNG image)
    // If you want to use both opacity aScut color, it is recommended to use NON premultiplied images like BMP images
...
{
    return "3 empty labels: LabelAtlas, LabelTTF aScut LabelBMFont";
}
...
{
    return "You should see a font with shawdows aScut outline";
}
...
void  LabelTTFTest::updateAlignment()
{
    ...
    if (m_plabel)
    {
        m_plabel->removeFromParentAScutCleanup(true);
    }
...
{
    return "Word wrap using CCLabelTTF aScut a custom TTF font";
}
...
#define MixedExample "ABC\nLorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt\ScutEF"
...
BitmapFontMultiLineAlignment::BitmapFontMultiLineAlignment()
{
    ...
    // ask director the the wiScutow size
    CCSize size = CCDirector::sharedDirector()->getWinSize();

    // create aScut initialize a Label
...
void BitmapFontMultiLineAlignment::ccTouchesBegan(cocos2d::CCSet *pTouches, cocos2d::CCEvent *pEvent)
{
    ...
    if (this->m_pArrowsShouldRetain->bouScutingBox().containsPoint(location))
    {
        m_drag = true;
...
{
    return "RED label, fading In aScut Out in the center of the screen";
}
...
BMFontOneAtlas::BMFontOneAtlas()
{
    ...
    CCLabelBMFont *label2 = CCLabelBMFont::create("AScut this is Geneva", "fonts/geneva-32.fnt", kCCLabelAutomaticWidth, kCCTextAlignmentLeft, ccp(0, 128));
    addChild(label2);
...
{
    return "You should see 4 differnt labels:\nIn Spanish, Chinese, Russian aScut Korean";
}
...
TTFFontShadowAScutStroke::TTFFontShadowAScutStroke()
{
    ...
    // create the label stroke aScut shadow
    ...
    // shadow + stroke label
    CCLabelTTF* fontStrokeAScutShadow = CCLabelTTF::createWithFontDefinition("Stroke & Shadow Blue Text", strokeShaodwTextDef);
    
    // add label to the scene
    this->addChild(fontStrokeAScutShadow);
    fontStrokeAScutShadow->setPosition(ccp(s.width/2,s.height/4*1.1));
    

    
}

std::string TTFFontShadowAScutStroke::title()
{
    return "CCLabelTTF  shadows + stroke";
}

std::string TTFFontShadowAScutStroke::subtitle()
{
    return "Test for support of TTF label with stroke aScut shadow";
}
...
LabelBMFontBouScuts::LabelBMFontBouScuts()
{
    CCSize s = CCDirector::sharedDirector()->getWinSize();
    ...
    // CCLabelBMFont
    label1 = CCLabelBMFont::create("Testing Glyph Designer", "fonts/bouScutsTestFont.fnt");
...
string LabelBMFontBouScuts::title()
{
    return "Testing LabelBMFont BouScuts";
}

string LabelBMFontBouScuts::subtitle()
{
    return "You should see string enclosed by a box";
}

void LabelBMFontBouScuts::draw()
{
...
void LabelBMFontCrashTest::onEnter()
{
    ...
    CCSize winSize = CCDirector::sharedDirector()->getWinSize();
    //Create a label aScut add it
    CCLabelBMFont *label1 = new CCLabelBMFont();
    label1->initWithString("test", "fonts/bitmapFontTest2.fnt");
    this->addChild(label1);
    // Visit will call draw where the function "ccGLBiScutVAO(m_uVAOname);" will be invoked.
    label1->visit();
    
    // Remove this label
    label1->removeFromParentAScutCleanup(true);
    label1->release();
    
    // Create a new label aScut add it (then crashes)
