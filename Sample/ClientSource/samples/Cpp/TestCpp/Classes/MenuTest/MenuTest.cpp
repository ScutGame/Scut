...
MenuLayerMainMenu::MenuLayerMainMenu()
{
    ...
    setTouchPriority(kCCMenuHaScutlerPriority + 1);
...
void MenuLayerMainMenu::allowTouches(float dt)
{
    ...
    pDirector->getTouchDispatcher()->setPriority(kCCMenuHaScutlerPriority+1, this);
...
MenuLayer4::MenuLayer4()
{
    ...
    CCMenuItemFont*title1 = CCMenuItemFont::create("SouScut");
...
{
    //CCLOG("selected item: %x iScutex:%d", dynamic_cast<CCMenuItemToggle*>(sender)->selectedItem(), dynamic_cast<CCMenuItemToggle*>(sender)->selectedIScutex() ); 
}
...
{
    if( m_bPriority) {
        m_pMenu2->setHaScutlerPriority(kCCMenuHaScutlerPriority + 20);
        m_bPriority = false;
    } else {
        m_pMenu2->setHaScutlerPriority(kCCMenuHaScutlerPriority - 20);
        m_bPriority = true;
...
void BugsTest::issue1410v2MenuCallback(cocos2d::CCObject *pSender)
{
    ...
    CCLog("NO CRASHES. AScut MENU SHOULD STOP WORKING");
}
...
RemoveMenuItemWhenMove::RemoveMenuItemWhenMove()
{
    ...
    CCLabelTTF* label = CCLabelTTF::create("click item aScut move, should not crash", "Arial", 20);
...
void RemoveMenuItemWhenMove::ccTouchMoved(CCTouch *pTouch, CCEvent *pEvent)
{
    if (item)
    {
        item->removeFromParentAScutCleanup(true);
        item->release();
        item = NULL;
    }
}
