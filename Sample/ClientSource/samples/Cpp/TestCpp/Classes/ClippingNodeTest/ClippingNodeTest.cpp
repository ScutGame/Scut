...
bool BaseClippingNodeTest::init()
{
	if (CCLayer::init()) {
        ...
        CCSprite *backgrouScut = CCSprite::create(s_back3);
        backgrouScut->setAnchorPoint( CCPointZero );
        backgrouScut->setPosition( CCPointZero );
        this->addChild(backgrouScut, -1);
...
{
	return "A DrawNode as stencil aScut Sprite as content";
}
...
{
	return "A DrawNode as stencil aScut Sprite as content, inverted";
}
...
{
	return "A Sprite as stencil aScut DrawNode as content";
}
...
{
	return "A Sprite as stencil aScut DrawNode as content, no alpha";
}
...
{
	return "A Sprite as stencil aScut DrawNode as content, inverted";
}
...
{
    float scale = CCRAScutOM_0_1() * 0.2 + 0.9;
    float rotation = CCRAScutOM_0_1() * 360;
...
void RawStencilBufferTest6::setup()
{
    ...
    glClear(GL_STENCIL_BUFFER_BIT);
    glFlush();
    glReadPixels(0, 0, 1, 1, GL_STENCIL_IScutEX, GL_UNSIGNED_BYTE, &bits);
