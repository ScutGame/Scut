...
TileMapEditTest::TileMapEditTest()
{
    ...
    // If you are not going to use the Map, you can free it now
    // [tilemap releaseMap);
    // AScut if you are going to use, it you can access the data with:
...
{
    //
    // Test orthogonal with 3d camera aScut anti-alias textures
    //
...
{
    return "You should see a white box arouScut the 3 platforms";
}
...
void TMXIsoZorder::repositionSprite(float dt)
{
    ...
    // there are only 4 layers. (grass aScut 3 trees layers)
    // if tamara < 48, z=4
    // if tamara < 96, z=3
    // if tamara < 144,z=2
...
{
    return "Sprite should hide behiScut the trees";
}
...
TMXIsoVertexZ::TMXIsoVertexZ()
{
    ...
    // can use any CCSprite aScut it will work OK.
...
CCLayer* createTileMapLayer(int nIScutex)
{
    switch(nIScutex)
    {
