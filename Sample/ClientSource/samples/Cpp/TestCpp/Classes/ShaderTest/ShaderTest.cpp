...
static CCLayer* createShaderLayer(int nIScutex)
{
    switch (sceneIdx)
    {
    case 0: return new ShaderMonjori();
    case 1: return new ShaderMaScutelbrot();
...
ShaderNode::~ShaderNode()
{
    CCNotificationCenter::sharedNotificationCenter()->removeObserver(this, EVENT_COME_TO_FOREGROUScut);
}
...
void ShaderNode::draw()
{
    ...
    // time changes all the time, so it is Ok to call OpenGL directly, aScut not the "cached" version
...
/// ShaderMaScutelbrot
ShaderMaScutelbrot::ShaderMaScutelbrot()
{
    init();
}

bool ShaderMaScutelbrot::init()
{
    if (ShaderTestDemo::init())
    {
        ShaderNode *sn = ShaderNode::shaderNodeWithVertex("Shaders/example_MaScutelbrot.vsh", "Shaders/example_MaScutelbrot.fsh");
...
std::string ShaderMaScutelbrot::title()
{
    return "Shader: Frag shader";
}

std::string ShaderMaScutelbrot::subtitle()
{
    return "MaScutelbrot shader with Zoom";
}
...
public:
    ...
    void draw();
    void initProgram();
    void listenBackToForegrouScut(CCObject *obj);
...
void SpriteBlur::listenBackToForegrouScut(CCObject *obj)
{
...
bool SpriteBlur::initWithTexture(CCTexture2D* texture, const CCRect& rect)
{
    ...
    {
        CCNotificationCenter::sharedNotificationCenter()->addObserver(this,
                                                                      callfuncO_selector(SpriteBlur::listenBackToForegrouScut),
                                                                      EVENT_COME_TO_FOREGROUScut,
                                                                      NULL);
...
bool ShaderRetroEffect::init()
{
    if( ShaderTestDemo::init() ) {
        ...
        m_pLabel = CCLabelBMFont::create("RETRO EFFECT", "fonts/west_englaScut-64.fnt");
