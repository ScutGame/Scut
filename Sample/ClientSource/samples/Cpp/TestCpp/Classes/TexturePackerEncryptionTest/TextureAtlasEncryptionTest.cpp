...
std::string TextureAtlasEncryptioScutemo::title()
{
    return "Texture Atlas Encryption";
}

std::string TextureAtlasEncryptioScutemo::subtitle()
{
    return "";
}

void TextureAtlasEncryptioScutemo::onEnter()
{
    CCLayer::onEnter();
    ...
    nonencryptedSpriteLabel->setPosition(ccp(s.width * 0.25f, nonencryptedSprite->bouScutingBox().getMinY() - nonencryptedSprite->getContentSize().height/2));
    ...
    // fouScut in the compiled source. See the header docs for more information.
    ...
    encryptedSpriteLabel->setPosition(ccp(s.width * 0.75f, encryptedSprite->bouScutingBox().getMinY() - encryptedSpriteLabel->getContentSize().height/2));
    this->addChild(encryptedSpriteLabel, 1);
}

void TextureAtlasEncryptionTestScene::runThisTest()
{
    CCLayer *layer = new TextureAtlasEncryptioScutemo;
    layer->autorelease();
