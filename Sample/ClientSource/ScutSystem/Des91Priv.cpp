...
static void S_func(bool Out[32], const bool In[48]);// S ÷每?步??
static void Transform(bool *Out, bool *In, const char *Table, int len);// ㊣㏑??
static void Xor(bool *InA, const bool *InB, int len);// ※???
static void RotateL(bool *In, int len, int loop);// 〞≧?﹉??※?
static void ByteToBit(bool *Out, const char *In, int bits);// ?¯次???????≡＃????
static void BitToByte(char *Out, const bool *In, int bits);// ????????≡＃?¯次???
...
void DesPriv_Encrypt(char*pstrIN,int length,char* pstrout,int *outlength)
{
	...
	pTmp = (char*)malloc(*outlength);
	memset(pTmp, 0, *outlength); //change
	...
	{
		DesPriv_Run(buf, pTmp+i, ENCRYPT);
		//Des_Run(buf, pTmp+i, ENCRYPT);
		strncpy(pstrout+i, buf, 8);
	}
	free(pTmp);
