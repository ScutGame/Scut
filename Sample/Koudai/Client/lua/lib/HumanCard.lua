...
function  HumanCard:createPlayer(parent,spr_name_string,speed,name,layerTag,Tag,quality)
...
function  HumanCard:init()
	local bgImg = "common/list_1032.png"
	if self._quality then
		bgImg = getQualityBg(self._quality, 4)
	end

	local itemBg = CCSprite:create(P(bgImg))
	...
	if self._SpritePath then
		local imageLabel = CCMenuItemImage:create(P(self._SpritePath),P(self._SpritePath))
	...
	if self._name then
		...
		local color = nil
		if self._quality == 0 then
			color = ZyColor:colorGreen()
		else
			color = ZyColor:getRoleQualityColor(self._quality)
		end
...
--设置是否显示
function HumanCard:setVisible(parent)
	self._sprite:setVisible(parent)
end;
...
--设置是否显示
function HumanCard:setVisible(value)
	self._sprite:setVisible(value)
end;
...
function HumanCard:dodge(funName)
	...
	local action1 = CCMoveBy:create(0.1, PT(SX(50),0))
	local action2 = CCMoveBy:create(0.2, PT(-SX(50),0))
	local action3 = CCSequence:createWithTwoActions(action1,action2)
	
	local action = CCSequence:createWithTwoActions(action3,CCCallFuncN:create(funName));	
...
function HumanCard:defend()
	action1 = CCMoveBy:create(0.1, PT(0,100))
	action2 = CCMoveBy:create(0.2, PT(0,-100))
	
	action3 = CCSequence:createWithTwoActions(action1,action2)	
...
function HumanCard:MomentumMax()
	self._momentum:setVisible(true)
end;

--气势不满
function HumanCard:MomentumNotMax()
	self._momentum:setVisible(false)
end;
