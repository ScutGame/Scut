...
function Human:move(moveX,moveY,callbackfun)
	...
	if moveX~=spriteX or moveY~=spriteY then
		...
		local moveAct = CCMoveTo:create(move_time,MoveTo);
		if callbackfun~=nil then
			moveAct=CCSequence:createWithTwoActions(moveAct, CCCallFuncN:create(callbackfun))
		end
		self:run()
		self:play()
...
--设置是否显示
function Human:setVisible(parent)
...
--设置是否显示
function Human:setVisible(value)
