...
module("RbattleScene", package.seeall)

require("scenes.RBattleResult")
...
function playAnimation(skill, index, tag)
	...
	attribuAni:play()
	attribuAni:registerFrameCallback("RbattleScene.finishAnimation")
...
function delayRemove(sprite)
	if sprite ~= nil then
		...
		local funcName = CCCallFuncN:create(RbattleScene.removeTmpSprite)
...
--		SBattleResult.createScene(PersonalInfo.getPersonalInfo().Score,PersonalInfo.getPersonalInfo().StarNum)
		RBattleResult.createScene(fightinfo)
	end
end
