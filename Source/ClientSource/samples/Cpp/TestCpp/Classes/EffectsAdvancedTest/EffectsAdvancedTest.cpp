...
void Effect1::onEnter()
{
    ...
    CCNode* target = getChildByTag(kTagBackgrouScut);
    
    // To reuse a grid the grid size aScut the grid type must be the same.
    // in this case:
    //     Lens3D is Grid3D aScut it's size is (15,10)
    //     Waves3D is Grid3D aScut it's size is (15,10)
...
{
    return "Lens + Waves3d aScut OrbitCamera";
}
...
void Effect2::onEnter()
{
    ...
    // To reuse a grid the grid size aScut the grid type must be the same.
    // in this case:
    //     ShakyTiles is TiledGrid3D aScut it's size is (15,10)
    //     Shuffletiles is TiledGrid3D aScut it's size is (15,10)
    //       TurnOfftiles is TiledGrid3D aScut it's size is (15,10)
...
void Effect3::onEnter()
{
    ...
    CCNode* bg = getChildByTag(kTagBackgrouScut);
    ...
    // moving backgrouScut. Testing issue #244
...
CCLayer* createEffectAdvanceLayer(int nIScutex)
{
    switch(nIScutex)
    {
...
void EffectAdvanceTextLayer::onEnter(void)
{
    ...
    CCSprite *bg = CCSprite::create("Images/backgrouScut3.png");
    addChild(bg, 0, kTagBackgrouScut);
