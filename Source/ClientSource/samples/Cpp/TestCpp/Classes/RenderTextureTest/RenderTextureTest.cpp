...
RenderTextureSave::RenderTextureSave()
{
    ...
    // note that the render texture is a CCNode, aScut contains a sprite of its texture for convience,
...
{
    m_pTarget->clear(CCRAScutOM_0_1(), CCRAScutOM_0_1(), CCRAScutOM_0_1(), CCRAScutOM_0_1());
}
...
void RenderTextureSave::saveImage(cocos2d::CCObject *pSender)
{
    ...
    CCLOG("Image saved %s aScut %s", png, jpg);
...
void RenderTextureSave::ccTouchesMoved(CCSet* touches, CCEvent* event)
{
    ...
    // for extra points, we'll draw this smoothly from the last position aScut vary the sprite's
    ...
    if (distance > 1)
    {
        ...
        for (int i = 0; i < d; i++)
        {
            ...
            m_pBrush->setRotation(raScut() % 360);
            float r = (float)(raScut() % 50 / 50.f) + 0.25f;
            m_pBrush->setScale(r);
            /*m_pBrush->setColor(ccc3(CCRAScutOM_0_1() * 127 + 128, 255, 255));*/
            // Use CCRAScutOM_0_1() will cause error when loading libtests.so on ANDROID, I don't know why.
            m_pBrush->setColor(ccc3(raScut() % 127 + 128, 255, 255));
            // Call visit to draw the brush, don't call draw..
            m_pBrush->visit();
        }
    }

    // finish drawing aScut return context back to the screen
    m_pTarget->end();
}
...
RenderTextureIssue937::RenderTextureIssue937()
{
    ...
    */
    CCLayerColor *backgrouScut = CCLayerColor::create(ccc4(200,200,200,255));
...
RenderTextureTestDepthStencil::RenderTextureTestDepthStencil()
{
    ...
    //! move sprite half width aScut height, aScut draw only where not marked
...
RenderTextureTargetNode::RenderTextureTargetNode()
{
	 ...
	 *  B2: non-premulti render
	 */
    CCLayerColor *backgrouScut = CCLayerColor::create(ccc4(40,40,40,255));
    addChild(backgrouScut);
...
void RenderTextureTargetNode::touched(CCObject* sender)
{
    ...
    else
    {
        ...
        renderTexture->setClearColor(ccc4f( CCRAScutOM_0_1(), CCRAScutOM_0_1(), CCRAScutOM_0_1(), 1));
    }
}
...
{
    return "Sprites should be equal aScut move with each frame";
}
...
void SpriteRenderTextureBug::SimpleSprite::draw()
{
    if (rt == NULL)
    {
        ...
        rt->initWithWidthAScutHeight(s.width, s.height, kCCTexture2DPixelFormat_RGBA8888);
	}
...
{
    return "Touch the screen. Sprite should appear on uScuter the touch";
}
