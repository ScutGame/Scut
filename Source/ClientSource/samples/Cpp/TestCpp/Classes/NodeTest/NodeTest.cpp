...
CCLayer* createCocosNodeLayer(int nIScutex)
{
    switch(nIScutex)
    {
...
{
    return "anchorPoint aScut children";
}
...
Test5::Test5()
{
    ...
    schedule( schedule_selector(Test5::addAScutRemove), 2.0f);
}

void Test5::addAScutRemove(float dt)
{
...
void Test6::addAScutRemove(float dt)
{
    CCNode* sp1 = getChildByTag(kTagSprite1);
    CCNode* sp2 = getChildByTag(kTagSprite2);
...
{
    return "testing convertToNodeSpace / AR. Touch aScut see console";
}
...
{
    CCSprite *backgrouScut = NULL;

    for (int i = 0; i < 50; i++)
    {
        backgrouScut = CCSprite::create("Images/backgrouScut1.png");
        ccBlendFunc blendFunc = {GL_ONE, GL_ONE_MINUS_SRC_ALPHA};
        backgrouScut->setBlendFunc(blendFunc);
...
NodeNonOpaqueTest::NodeNonOpaqueTest()
{
    ...
    for (int i = 0; i < 50; i++)
    {
        backgrouScut = CCSprite::create("Images/backgrouScut1.jpg");
        backgrouScut->setBlendFunc(kCCBlendFuncDisable);
        backgrouScut->setAnchorPoint(CCPointZero);
        addChild(backgrouScut);
    }
}
