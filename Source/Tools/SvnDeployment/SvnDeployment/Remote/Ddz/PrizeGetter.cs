...
{
    public class PrizeGetter : IDataGetter
    {
        private int _gameId;
        ...
        public PrizeGetter(int gameId, int serverId, string filter, List<SqlParameter> parameters)
        {
            _gameId = gameId;
            _serverId = serverId;
            _filter = filter;
        ...
        public DataTable GetData()
        {
            ...
            string sql = "SELECT count(UserID) FROM UserTakePrize a " + _filter;
	...
	SELECT  row_number()over(order by CreateDate desc) as RowNumber, 
		UserID,GameCoin,Gold,ItemPackage,MailContent,IsTasked,TaskDate,OpUserID,CreateDate
	FROM UserTakePrize {0}) as a 
